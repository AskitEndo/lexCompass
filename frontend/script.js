document.addEventListener("DOMContentLoaded", () => {
  ...
  documentUpload.addEventListener("change", (e) => {
    ...
    if (file) {
      ...
      label.className = "material-button bg-tertiary text-white cursor-pointer inline-block";
      analyzeBtn.disabled = false;
      analyzeBtn.classList.remove("opacity-50", "cursor-not-allowed");
    } else {
      label.innerHTML = "📁 Choose Document";
      label.className = "material-button bg-primary text-white hover:bg-blue-700 cursor-pointer inline-block";
  ...
  analyzeBtn.addEventListener("click", async () => {
    ...
    if (!file) {
      showMaterialNotification("Please select a .txt file to analyze.", "error");
      return;
    }
  ...
  function displayResults({ decisionMap, riskRadar }) {
    ...
    decisionMap.forEach((item, index) => {
      ...
      card.className = "bg-white border border-gray-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow";
      card.innerHTML = `
      ...
      riskCard.innerHTML = `
        <div class="flex justify-between items-start mb-4">
          <div class="flex items-center space-x-2">
            ...
            <h4 class="text-lg font-semibold text-gray-800">Risk Assessment #${index + 1}</h4>
          </div>
          <div class="flex items-center space-x-2">
        ...
        <button 
          ...
          onclick="getMaterialCoachSuggestion('${risk.clause.replace(/'/g, "\\'")}')"
        >
  ...
  window.exportMaterialAnalysis = function() {
    const analysisData = {
      ...
      risks: Array.from(riskRadarDiv.children).map(card => {
        const clause = card.querySelector('blockquote').textContent.replace(/"/g, '');
      ...
      coachSuggestion: coachResultDiv.querySelector('blockquote')?.textContent || null,
      coachExplanation: coachResultDiv.querySelector('.text-gray-700')?.textContent || null
    };
